...
function generateInsights(
    ...
    if (metrics.lost_is_budget > 0.2) {
        insights.push({
            ...
            what: `You're losing ${((metrics.lost_is_budget || 0) * 100).toFixed(0)}% of impression share due to budget constraints.`,
            ...
            impact: `Could increase impressions by up to ${((metrics.lost_is_budget || 0) * 100).toFixed(0)}%`,
        });
    }
    ...
    if (metrics.avg_quality_score < 0.5) {
        insights.push({
            ...
            what: `Your average Quality Score is ${((metrics.avg_quality_score || 0) * 10).toFixed(1)}/10, which is below average.`,
    ...
    if (metrics.lost_is_rank > 0.3) {
        insights.push({
            ...
            what: `You're losing ${((metrics.lost_is_rank || 0) * 100).toFixed(0)}% of eligible impressions due to ad rank.`,
            ...
            impact: `Could capture an additional ${((metrics.lost_is_rank || 0) * 100 * 0.5).toFixed(0)}% of impressions`,
        });
    }
    ...
    if (metrics.ctr < 0.02) {
        insights.push({
            ...
            what: `Your CTR of ${((metrics.ctr || 0) * 100).toFixed(2)}% is below the typical 2-5% range.`,
    ...
    if (metrics.cvr < 0.02 && metrics.ctr > 0.02) {
        insights.push({
            ...
            what: `Your CVR of ${((metrics.cvr || 0) * 100).toFixed(2)}% means most clicks don't convert.`,
    ...
    if (metrics.cpc > 10) {
        insights.push({
            ...
            what: `Your average CPC of $${(metrics.cpc || 0).toFixed(2)} is relatively high for this market.`,
